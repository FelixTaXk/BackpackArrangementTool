...
//    （品质底色/图例色块填充选择器依赖）；图例各色块 .sw 同步补设 data-quality，
//    由 styles.css 按品质令牌填充，保持图例与格子语义一致。
// 2) 包装 setSolverRunning：solver.js 运行态会改写 solveBtn 文本，结束后恢复初始双行结构（运行期快照）。
// 本文件不改变任何求解/存档/消息契约，只做只读查询与 DOM 属性/样式装饰。
'use strict';

// 属性 id 仅作只读反查透传（data-attr 保留，供后续属性维度装饰使用）。
...
// 结果格补 data-attr / data-quality：#resultGrid 子元素按行优先（r*W+c）排布，与 renderResultGrid 生成顺序一致。
function themeDecorateResultGrid(best){
  ...
  for(const p of best.placements){
    ...
    const qid = QUALITY_NAME_TO_ID[p.quality] || '';
    for(const cell of p.cells){
      ...
      // 仅对结果项格补属性：防锁定格/空格被挂上属性/品质。
      if(!div || !div.classList.contains('item-cell')) continue;
      if(attr) div.setAttribute('data-attr', attr);
      if(qid) div.setAttribute('data-quality', qid);
    }
  }
}

// 图例 .sw 补 data-quality：结果格按品质填充底色，图例同步按品质填色保持语义一致。
// 图例 tag 与 placements 的对应关系（只读参考 ui-result.js L53-62）：tag 文本以 "#编号" 开头，
// 按编号升序排序；此处按 no 反查 placement 品质，补 data-quality 供 styles.css 品质选择器填充。
function themeRecolorLegend(best){
  const legend = document.getElementById('legend');
  if(!legend || !best || !Array.isArray(best.placements) || !best.placements.length) return;
  const qualityByNo = new Map();
  for(const p of best.placements){
    const qid = QUALITY_NAME_TO_ID[p.quality] || '';
    if(qid) qualityByNo.set(String(p.no), qid);
  }
  if(!qualityByNo.size) return;
  legend.querySelectorAll('.tag').forEach(tag=>{
    const m = (tag.textContent || '').match(/^#(\d+)/);
    if(!m) return;
    const qid = qualityByNo.get(m[1]) || '';
    if(!qid) return;
    const sw = tag.querySelector('.sw');
    if(!sw) return;
    sw.setAttribute('data-quality', qid);
  });
}
